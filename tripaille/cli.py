...
from tripaille import __version__  # noqa, ditto

CONTEXT_SETTINGS = dict(auto_envvar_prefix='TRIPAILLE', help_option_names=['-h', '--help'])
...
class Context(object):
    ...
    def exit(self, exit_code):
        self.vlog("Exiting tripaille with exit code [%d]" % exit_code)
        sys.exit(exit_code)
...
@click.command(cls=tripailleCLI, context_settings=CONTEXT_SETTINGS)
...
@click.option(
    "-i",
    "--instance",
    help='Name of instance in %s. This parameter can also be set via the environment variable TRIPAILLE_INSTANCE' % global_config_path(),
    default='__default',
    show_default=True,
    required=True
)
...
@pass_context
def tripaille(ctx, instance, verbose, path=None):
    """Command line wrappers around Python-Tripal functions."""
