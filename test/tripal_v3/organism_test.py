...
class OrganismTest(unittest.TestCase):
    ...
    def test_add_organism(self):
        ...
        assert org["@id"].startswith("http://"), "ana properly created"
        assert org["ItemPage"].startswith("http://"), "ana properly created"
        ...
        assert len(org) > 0, "org entity properly created"

        found_org = False
        for org in org['member']:
            if org['label'] == '%s %s ' % (genus, species):
                found_org = True

        assert found_org, "org entity properly created"
