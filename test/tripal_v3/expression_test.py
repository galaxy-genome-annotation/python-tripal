...
class ExpressionTest(unittest.TestCase):
    ...
    def test_sync_biomaterials(self):
        ...
        assert biomat_published_dict["totalItems"] == 0, "Error : Number of published biomaterials is not 0"

        self.ti.entity.publish(types=["Biological Sample"])
        biomat_published_dict = self.ti.expression.get_biomaterials_tripal()

        assert biomat_published_dict["totalItems"] == 1, "Error : Number of published biomaterials is not 1"
        assert biomat_published_dict["member"][0]["label"] == biomaterial_name, "Error : Published biomaterial label does not match"
